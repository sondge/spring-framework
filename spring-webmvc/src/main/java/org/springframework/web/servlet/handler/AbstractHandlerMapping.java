 ...
 * <p>
 * HandlerMapping 的抽象事项，支持排序，默认处理器，处理拦截器，包括处理拦截器映射通过路劲匹配
 *
...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 * <p>
	 * 添加方式有两种：
	 * {@link #setInterceptors(Object...)} 方法
	 * {@link #extendInterceptors(List)} 方法
	 */
	 ...
	 * <p>
	 * 根据给定 HandlerMapping 设置为默认的处理器
	 * This handler will be returned if no specific mapping was found.
	 * 如果没有指定的路由被发现这个处理器将被返回
	 ...
	 * 初始化拦截器
	 * 该方法是对 WebApplicationObjectSupport 的重写
	 * WebApplicationObjectSupport 的集成关系为：
	...
	protected void initApplicationContext() throws BeansException {
		...
		// 探测已注册的 MappedInterception 的 Bean 们，添加到 MappedInterceptor
		detectMappedInterceptors(this.adaptedInterceptors);
		// 将 Interceptor 初始化为 HandlerInterceptor 类型，添加到 mappedInterceptors 中
		initInterceptors();
	}
	...
	protected void detectMappedInterceptors(List<HandlerInterceptor> mappedInterceptors) {
		// 扫描已经注册的 MappedInterceptor 的 Bean 们，添加到 mappedInterceptors 中
	...
	protected void initInterceptors() {
		...
		if (!this.interceptors.isEmpty()) {
			...
			for (int i = 0; i < this.interceptors.size(); i++) {
				...
				// 将 Intercepts 初始化成 HandlerInterceptor 类型， 添加到 mappedInterceptor 中
	...
	protected HandlerInterceptor adaptInterceptor(Object interceptor) {
		// 如果 interceptor 是 HandlerInterceptor 类型的
		if (interceptor instanceof HandlerInterceptor) {
			return (HandlerInterceptor) interceptor;
			// 如果是 WebRequestInterceptor 类型的
		} else if (interceptor instanceof WebRequestInterceptor) {
			// WebRequestHandlerInterceptor，适配成 WebRequestHandlerInterceptorAdapter 类型
	 ...
	 * 给定的拦截器查找一个新的拦截器回退成默认的拦截器，如果没有指定的拦截器
	 *
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		...
		// 如果 handler 为空，则直接返回空
		if (handler == null) {
			return null;
		}
		// Bean name or resolved handler?
		// 如果 handler 是 String 类型的
